...
live dashboard와 `--once` 출력은 실행 중인 이슈마다 현재 stage뿐 아니라 tracker state, 현재 activity, session/pid/tokens runtime, 최근 이벤트 목록까지 함께 보여준다. 따라서 어떤 에이전트가 지금 어떤 tool call을 실행 중인지, Codex app-server가 어떤 서버 요청/알림/stderr를 내보냈는지 터미널에서 바로 확인할 수 있다.
