...
func (s *Server) handleDashboard(w http.ResponseWriter, r *http.Request) {
	...
	totalRunning, totalRetrying, totalFailureRetrying, totalCapacityWaiting, totalCompleted := 0, 0, 0, 0, 0
	...
	for proj, st := range states {
		...
		failureRetrying, capacityWaiting := countRetriesByKind(st.RetryQueue)
		totalFailureRetrying += failureRetrying
		totalCapacityWaiting += capacityWaiting
	...
	fmt.Fprintf(w, dashboardHTML, totalRunning, totalRetrying, totalFailureRetrying, totalCapacityWaiting, totalCompleted, totalTokens, rows)
}
...
func (s *Server) handleState(w http.ResponseWriter, r *http.Request) {
	...
	for _, st := range states {
		...
		for _, e := range st.RetryQueue {
			retrying[e.Identifier] = map[string]any{
				"attempt":       e.Attempt,
				"kind":          e.Kind,
				"failure_count": e.FailureCount,
				"defer_count":   e.DeferCount,
				"error":         e.Error,
			}
		}
...
func (s *Server) handleProjects(w http.ResponseWriter, r *http.Request) {
	...
	for name, st := range states {
		...
		result = append(result, map[string]any{
			...
			"failure_retrying": failureRetrying,
			"capacity_waiting": capacityWaiting,
...
func countRetriesByKind(retries map[string]*orchestrator.RetryEntry) (failureRetrying int, capacityWaiting int) {
	...
	return failureRetrying, capacityWaiting
}
...
<div>
  ...
  <div class="stat"><div class="stat-val">%d</div><div class="stat-label">Failure Retry</div></div>
  <div class="stat"><div class="stat-val">%d</div><div class="stat-label">Capacity Wait</div></div>
