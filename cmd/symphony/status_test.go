...
func TestFormatStatusSummaryIncludesRunningIssueDetails(t *testing.T) {
	...
	for _, want := range []string{
		...
		"J-54 | streaming_turn | attempt 2 | turn 3 | last event 2026-03-09T01:00:00Z",
		"tracker state: In Progress",
		"branch: j-54-status-detail",
		"execution: failures 1 | continuation pending | urgent",
...
func TestRenderStatusDashboardIncludesRunningAndBackoff(t *testing.T) {
	...
	for _, want := range []string{
		...
		"attempt: 2",
		"branch: j-76-status-progress-detail",
		"execution: failures 1 | continuation run | preempted",
